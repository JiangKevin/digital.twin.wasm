...
namespace FM
{
    ...
    template < typename Graph > class Polylines_visitor
    {
    private:
        std::vector< std::vector< Point_3 > >& polylines;
        Graph&                                 graph;
    public:
        Polylines_visitor( Graph& graph, std::vector< std::vector< Point_3 > >& polylines ) : polylines( polylines ), graph( graph ) {}
    ...
    {
        const std::string fname = CGAL::data_file_path( "points_3/b9_training.ply" );
        // Read points
        ...
        TIN_with_info tin_with_info( boost::make_transform_iterator( points.begin(), idx_to_point_with_info ), boost::make_transform_iterator( points.end(), idx_to_point_with_info ) );
        ...
        Mesh::Property_map< Mesh::Face_index, CGAL::IO::Color > color_map = tin_colored_mesh.add_property_map< Mesh::Face_index, CGAL::IO::Color >( "f:color" ).first;

        CGAL::copy_face_graph( tin_with_info, tin_colored_mesh,
                               CGAL::parameters::face_to_face_output_iterator( boost::make_function_output_iterator(
                                   [ & ]( const std::pair< TIN_with_info::Face_handle, Mesh::Face_index >& ff )
                                   {
                                       ...
                                       else
                                       {
                                           ...
                                           color_map[ ff.second ] = CGAL::IO::Color( r.get_int( 64, 192 ), r.get_int( 64, 192 ), r.get_int( 64, 192 ) );
                                       }
                                   } ) ) );
        ...
        {
            TIN_with_info::Face_circulator circ = tin_with_info.incident_faces( vh ), start = circ;
        ...
        CGAL::copy_face_graph( tin_with_info, dtm_mesh,
                               CGAL::parameters::face_to_face_output_iterator( boost::make_function_output_iterator(
                                   [ & ]( const std::pair< TIN_with_info::Face_handle, Mesh::Face_index >& ff )
                                   {
                                       ...
                                       for ( int i = 0; i < 3; ++i )
                                       {
                                           longest_edge = ( std::max )( longest_edge, CGAL::squared_distance( ff.first->vertex( ( i + 1 ) % 3 )->point(), ff.first->vertex( ( i + 2 ) % 3 )->point() ) );

                                           TIN_with_info::Face_circulator circ = tin_with_info.incident_faces( ff.first->vertex( i ) ), start = circ;
                                           do
                                           {
        ...
        for ( Mesh::Halfedge_index hi : holes )
        {
            ...
            double size = CGAL::squared_distance( Point_2( hole_bbox.xmin(), hole_bbox.ymin() ), Point_2( hole_bbox.xmax(), hole_bbox.ymax() ) );
            if ( size > max_size )
            {
                max_size   = size;
                outer_hull = hi;
        ...
        for ( Mesh::Halfedge_index hi : holes )
        {
            if ( hi != outer_hull )
            {
                CGAL::Polygon_mesh_processing::triangulate_refine_and_fair_hole( dtm_mesh, hi, CGAL::parameters::fairing_continuity( 0 ) );
            }
        }
        ...
        for ( std::size_t y = 0; y < height; ++y )
            ...
            {
                Point_3 query( bbox.xmin() + x * ( bbox.xmax() - bbox.xmin() ) / double( width ), bbox.ymin() + ( height - y ) * ( bbox.ymax() - bbox.ymin() ) / double( height ),
                ...
                {
                    std::array< double, 3 > barycentric_coordinates = CGAL::Polygon_mesh_processing::barycentric_coordinates( Point_2( location->vertex( 0 )->point().x(), location->vertex( 0 )->point().y() ), Point_2( location->vertex( 1 )->point().x(), location->vertex( 1 )->point().y() ),
                                                                                                                              Point_2( location->vertex( 2 )->point().x(), location->vertex( 2 )->point().y() ), Point_2( query.x(), query.y() ), Kernel() );

                    double height_at_query = ( barycentric_coordinates[ 0 ] * location->vertex( 0 )->point().z() + barycentric_coordinates[ 1 ] * location->vertex( 1 )->point().z() + barycentric_coordinates[ 2 ] * location->vertex( 2 )->point().z() );
        ...
        for ( TIN::Vertex_handle vh : dtm_clean.finite_vertex_handles() )
        {
            ...
            TIN::Vertex_circulator circ = dtm_clean.incident_vertices( vh ), start = circ;
        ...
        std::cerr << polylines.size() << " polylines computed, with " << map_p2v.size() << " vertices in total" << std::endl;
        ...
        std::size_t nb_vertices = std::accumulate( polylines.begin(), polylines.end(), std::size_t( 0 ),
                                                   []( std::size_t size, const std::vector< Point_3 >& poly ) -> std::size_t
                                                   {
                                                       return size + poly.size();
                                                   } );

        std::cerr << nb_vertices << " vertices remaining after simplification (" << 100. * ( nb_vertices / double( map_p2v.size() ) ) << "%)" << std::endl;
        ...
        if ( training_map.has_value() )
        {
            ...
            Classification::Point_set_feature_generator< Kernel, Point_set, Point_set::Point_map > generator( points, points.point_map(), 5 );  // 5 scales
            ...
            Classification::classify_with_graphcut< Concurrency_tag >( points, points.point_map(), labels, classifier,
            ...
            // Evaluate
            std::cerr << "Mean IoU on training data = " << Classification::Evaluation( labels, points.range( training_map.value() ), points.range( label_map ) ).mean_intersection_over_union() << std::endl;
