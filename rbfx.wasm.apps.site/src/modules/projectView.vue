    ...
    <div class="main_container_content_max blur_div_95">
        ...
        <div class="main_container_content">
            <v-layout full-height>
                                ...
                                <v-item v-slot="{ isSelected, toggle }">
                                    <v-hover>
                                        <template v-slot:default="{ isHovering, props }">
                                            <v-card v-bind="props" class="mx-auto opacity-70"
                                                :color="isHovering ? 'fm_card_select' : 'fm_card'" rounded="0"
                                                density="compact" style="user-select: none">
                                                <v-card-title> {{ n.fileName + " [" + n.Id + "]" }} </v-card-title>
                                                <v-card-subtitle>{{ n.stats.birthtime + " / " + n.stats.mtime
                                                    }}</v-card-subtitle>
                                                ...
                                                <v-divider></v-divider>
                                                <v-card-actions>
                                                    <v-btn icon="mdi-delete" @click="n.toDel = true" size="small"
                                                    ...
                                                    <v-btn icon="mdi-eye" @click="open_dt_project(n)" size="small"
                                                        ...
                                                        density="comfortable"></v-btn>
                                                    <v-btn icon="mdi-image-search" @click="texture_import_ck(n)"
                                                        size="small" :variant="isHovering ? 'flat' : 'text'"
                                                        :color="isHovering ? 'fm_red' : 'fm_white'"
                                                        density="comfortable"></v-btn>
                                                    <v-btn icon="mdi-cube-scan" @click="model_import_ck(n)" size="small"
                                                        :variant="isHovering ? 'flat' : 'text'"
                                                        :color="isHovering ? 'fm_red' : 'fm_white'"
                                                        density="comfortable"></v-btn>
                                                </v-card-actions>
                                                <v-expand-transition>
                                                    <v-card v-if="n.toDel" class="position-absolute w-100" height="100%"
                                                        style="bottom: 0" color="fm_card_ext" rounded="0">
                                                        <v-card-text class="pb-0">
                                                            <p class="text-h5">Delete Project</p>
                                                            <v-divider></v-divider>

                                                            <p class="text-medium-emphasis">Confirm to delete the
                                                                current project. Once deleted, it cannot be restored.
                                                                The deleted data includes resource files, project
                                                                descriptions, scene configurations, etc.</p>
                                                        </v-card-text>
                                                        <div class="submit_contain">
                                                            <v-card-actions class="fm_v_card_actions">
                                                                <v-btn class="ml-auto submit_btn" text="Delete"
                                                                    variant="elevated"
                                                                    @click="del_project_click(n)"></v-btn>
                                                                <v-btn class="ml-auto submit_btn" text="Close"
                                                                    variant="elevated" @click="n.toDel = false">
                                                                </v-btn>
                                                            </v-card-actions>
                                                        </div>
                                                    </v-card>
                ...
                <v-navigation-drawer location="right" permanent temporary v-model="mainStore_project.drawer"
                    width="400">
                    <!--  -->
                    <div v-show="mainStore_project.project_modify_type == 'CreateProject'" class="r_modify_div">
                        <v-text-field prepend-icon="mdi-black-mesa" label="Name" variant="outlined"
                            v-model="mainStore_project.project_info.name" rounded="0" density="compact"></v-text-field>
                        <v-textarea prepend-icon="mdi-hololens" label="Remark" variant="outlined"
                            v-model="mainStore_project.project_info.info" rounded="0" density="compact"></v-textarea>
                        <v-select prepend-icon="mdi-database-search" label="Template" variant="outlined"
                            v-model="mainStore_project.project_info.wizard" :items="mainStore_project.wizard_list"
                            rounded="0" density="compact" class="mx-auto"> </v-select>
                        <v-file-input id="upload_inputs" ref="upload_inputs" multiple
                            label="Select Resouse Files( ZIP File )" accept=".zip" variant="outlined" rounded="0"
                            density="compact"></v-file-input>
                        <div class="submit_contain">
                            <v-card-actions class="fm_v_card_actions">
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Create"
                                    @click="create_and_uploadFiles"></v-btn>
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Close"
                                    @click="close_project_drawer"></v-btn>
                            </v-card-actions>
                        </div>
                    </div>
                    <!--  -->
                    <div v-show="mainStore_project.project_modify_type == 'ModelImport'" class="r_modify_div">
                        <v-select prepend-icon="mdi-black-mesa" label="Project" variant="outlined"
                            v-model="mainStore_project.resource_info.project" :items="mainStore_project.project_list"
                            rounded="0" density="compact"></v-select>
                        <v-select prepend-icon="mdi-powershell" label="Command" variant="outlined"
                            v-model="mainStore_project.res_import_commond_info.command"
                            :items="mainStore_project.res_import_commond_list" rounded="0" density="compact"></v-select>
                        <v-file-input id="model_upload_inputs" ref="model_upload_inputs"
                            label="Select Resouse Files( FBX File )" accept=".fbx" variant="outlined"
                            prepend-icon="mdi-cube-scan" rounded="0" density="compact"></v-file-input>
                        <v-text-field prepend-icon="mdi-cards-diamond" label="Parameter" variant="outlined"
                            v-model="mainStore_project.res_import_commond_info.parameter" rounded="0"
                            density="compact"></v-text-field>

                        <div class="submit_contain">
                            <v-card-actions class="fm_v_card_actions">
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Import"
                                    @click="model_import_do"></v-btn>
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Close"
                                    @click="close_project_drawer"></v-btn>
                            </v-card-actions>
                        </div>
                    ...
                    <div v-show="mainStore_project.project_modify_type == 'TextureImport'" class="r_modify_div">
                        <v-select prepend-icon="mdi-black-mesa" label="Project" variant="outlined"
                            v-model="mainStore_project.resource_info.project" :items="mainStore_project.project_list"
                            rounded="0" density="compact"></v-select>
                        <v-text-field prepend-icon="mdi-cards-diamond" label="Target Path" variant="outlined"
                            v-model="mainStore_project.res_import_des_info.desPath" rounded="0"
                            density="compact"></v-text-field>
                        <v-file-input id="texture_upload_inputs" ref="texture_upload_inputs" multiple
                            label="Select Resouse Files( Texture File Or Other )" accept=".png" variant="outlined"
                            prepend-icon="mdi-image-search" rounded="0" density="compact"></v-file-input>
                        <div class="submit_contain">
                            <v-card-actions class="fm_v_card_actions">
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Import"
                                    @click="texture_import_do"></v-btn>
                                <v-btn class="ml-auto submit_btn" variant="elevated" rounded="0" text="Close"
                                    @click="close_project_drawer"></v-btn>
                            </v-card-actions>
                        </div>
